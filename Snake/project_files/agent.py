...
class GameAIAgent:
    def __init__(self):
        self.memory = deque(maxlen=MAX_MEMORY) # 设置存储五元组的存储器

        self.model = Model(11, 256, 3) # 设置网络模型
    ...
    def train_long_memory(self): # 拿多组数据进行训练
        ...
        self.trainer.train_step(states, actions, rewards, next_states, game_overs) # 用多组数据（矩阵形式）进行训练
