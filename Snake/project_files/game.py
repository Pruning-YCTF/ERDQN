...
class Game:  # 游戏界面及游戏设置
    def __init__(self, speed=SPEED, Train=True):  # 速度作为参数输入
        ...
        self.food_list_1 = [Point(26 * 20, 13 * 20), Point(23 * 20, 7 * 20), Point(17 * 20, 3 * 20), Point(5 * 20, 8 * 20),
                            Point(1, 16 * 20), Point(4 * 20, 21 * 20), Point(12 * 20, 18 * 20), Point(19 * 20, 22 * 20),
                            Point(23 * 20, 16 * 20), Point(31 * 20, 23 * 20)]
        self.food_list_2 = [Point(28 * 20, 11 * 20), Point(24 * 20, 6 * 20), Point(17 * 20, 6 * 20), Point(8 * 20, 2 * 20),
                            Point(0, 6 * 20), Point(0, 23 * 20), Point(6 * 20, 20 * 20), Point(16 * 20, 20 * 20),
                            Point(26 * 20, 15 * 20), Point(31 * 20, 23 * 20)]
        self._place_a_wall()
        # 初始的蛇，分数，食物
        self.reset()  # 重置游戏参数

    def reset(self):  # 重置游戏参数的函数
        self.snake = Snake(initial_size=3)  # 蛇的长度重置为3
    ...
    def _place_a_wall(self):
         # env1
        for i in range(3):
            self.walls.append(Point((3 + i) * 20, 14 * 20))
        for i in range(6):
            self.walls.append(Point(6 * 20, (14 + i) * 20))

        # 右下角那一块
        for i in range(4):
            self.walls.append(Point((20 + i) * 20, 17 * 20))
        for i in range(4):
            self.walls.append(Point(20 * 20, (18 + i) * 20))

        # 上面一块
        for i in range(3):
            self.walls.append(Point((7 + i) * BLOCK_SIZE, 8 * BLOCK_SIZE))
        ...
        for i in range(5):
            ...
            self.walls.append(Point(26 * 20, (8 + i) * 20))  # 竖块2
        for i in range(3):
